...
class ExternalName < ActiveRecord::Base
  ...
  def self.exact_search(opts)
    quote_term = self.connection.quote(opts[:term])
    ExternalName.find_by_sql("SELECT DISTINCT en.*
      ...
      [self.connection.quote(opts[:scientific_name]), 
       quote_term, quote_term])
  end
  
  def self.like_search(opts)
    escape_term = self.sanitize(opts[:term]).gsub(/^'(.+)'$/, '\1')
    escape_scientific_name = self.sanitize(opts[:scientific_name])
    q = "SELECT DISTINCT en.*
      ...
      [escape_scientific_name, escape_term, 
       escape_term, escape_term, opts[:limit].to_i]
  ...
  def table_items
    Seabase::Normalizer.new(Replicate.all(), transcripts,  
